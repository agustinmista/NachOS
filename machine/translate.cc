...
Machine::ReadMem(int addr, int size, int *value)
{
    ...
    DEBUG('n', "Reading VA 0x%x, size %d\n", addr, size);
...
Machine::WriteMem(int addr, int size, int value)
{
    ...
    DEBUG('n', "Writing VA 0x%x, size %d, value 0x%x\n", addr, size, value);
...
Machine::Translate(int virtAddr, int* physAddr, int size, bool writing)
{
    ...
    DEBUG('n', "\tTranslate 0x%x, %s: ", virtAddr, writing ? "write" : "read");

// check for alignment errors
    if (((size == 4) && (virtAddr & 0x3)) || ((size == 2) && (virtAddr & 0x1))){
	DEBUG('n', "alignment problem at %d, size %d!\n", virtAddr, size);
...
// from the virtual address
	...
	if (vpn >= pageTableSize) {
	    DEBUG('n', "virtual page # %d too large for page table size %d!\n", 
	...
	if (entry == NULL) {				// not found
    	    DEBUG('n', "*** no valid TLB entry found for this virtual page!\n");
	        stats->numPageFaults++;
    ...
    if (entry->readOnly && writing) {	// trying to write to a read-only page
    	DEBUG('n', "%d mapped read-only at %d in TLB!\n", virtAddr, i);
