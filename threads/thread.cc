...
Thread::Thread(const char* threadName, int threadPriority)
{
    ...
    stack = NULL;
    status = JUST_CREATED;
    port = new Puerto(threadName);
    priority = threadPriority < 0 ? 0 : threadPriority;
#ifdef USER_PROGRAM
    space = NULL;
#endif
}
...
//----------------------------------------------------------------------
// Thread::Join
//	Locks the caller thread until this thread ends
//----------------------------------------------------------------------
...
//---------------------------------------------------------
//  Thread::openFile
//      busca un fd libre y guarda el puntero a este en
//      el arreglo de archivos abiertos, si no hay un 
//      fd libre devuelve -1
//      NOTA: 0 y 1 estan reservados para stdin y stdout      
//---------------------------------------------------------
OpenFileId Thread::getFileDescriptor (OpenFile *file) {
...
//---------------------------------------------------------
//  Thread::getFile
//      devuelve el puntero del archivo que corresponde
//      a fd, o null si está cerrado o es invalido.
//---------------------------------------------------------
OpenFile *Thread::getOpenFile(OpenFileId fd) {
    if(fd >= 2 && fd < MAX_FD)
        return openFiles[fd];
    else
        return NULL;
...
//-----------------------------------------
//  Thread::closeFile
//      si el archivo con fd esta abierto lo quita del
//      arreglo de archivos abiertos.
//      devuelve 0 si todo bien, -1 si no estaba
//      abierto o falla si se usa un fd invalido o 
//      se intenta cerrar ConsoleInput o ConsoleOutput
//-----------------------------------------
int Thread::freeFileDescriptor(OpenFileId fd) {
    if(fd < 2 || fd >= MAX_FD) 
        return -1;
