...
class List {
  public:
    ...
    bool RemItem(Item anItem); //Take given item off the list
...
// 
// Returns:
//	true if the element was in the list, false if it wasn't.
//----------------------------------------------------------------------

template <class Item>
bool
List<Item>::RemItem(Item anItem)
{
    if (IsEmpty()) return false;
    if (first->item == anItem){
		Remove();
	    return true;
	}
    for (ListNode *ptr = first; ptr != last; ptr = ptr->next) {
       if (ptr->next->item == anItem){
			ListNode *aux = ptr->next;
			ptr->next = ptr->next->next;
			if(aux == last) last = ptr;
			delete aux;
			return true;
	   }
    }
